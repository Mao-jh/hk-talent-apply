...
收到 `POST https://airpriv.top/api/hk-talent-apply` 的 `402` 响应后：
...
- 同时保存：资源 URL（`https://airpriv.top/api/hk-talent-apply`）、method（`POST`）、
  原请求 body JSON（`--data` 参数要原样复用）。
- 从本次用户请求生成 `--intent-summary`，如：`原始请求：优才高才申请获批画像预审（用户个人情况）`。
...
```bash
alipay-bot 402-buyer-pay --session-id '<sessionId>' --file '<第1步保存的账单文件>' --resource-url 'https://airpriv.top/api/hk-talent-apply' --intent-summary '原始请求：优才高才申请获批画像预审' --method 'POST' --data '<原请求body JSON>'
```

- `--data` 必须与原请求完全一致（含引号时注意 shell 转义）。
- 命令会输出支付二维码/支付卡片 → 原样转述给用户，请用户用支付宝扫码支付（¥4.99）。
...
alipay-bot 402-query-payment-status --trade-no '<交易号>' --resource-url 'https://airpriv.top/api/hk-talent-apply' --method 'POST' --data '<原请求body JSON>'

# 仅当输出含 32 位数字（第 11–14 位为 8282）的「订单号/查询单号」时可用：
alipay-bot 402-query-payment-status --out-shake-no '<查询单号>' --resource-url 'https://airpriv.top/api/hk-talent-apply' --method 'POST' --data '<原请求body JSON>'
```
